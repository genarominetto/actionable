...
class stepsScreen(Screen):
    class Step:
        ...
        def _get_step_info(self):
            with sqlite3.connect("tasks.db") as conn:
                ...
                # Fetch the latest 'Started' STEP_ID from HISTORY
                ...
                # Fetch the details of the current step
                ...
                # Calculate total steps for the task
        ...
        def _record_history(self):
            with sqlite3.connect("tasks.db") as conn:
                cursor = conn.cursor()
                cursor.execute("INSERT INTO HISTORY (STEP_ID, ACTION, TIME) VALUES (?, ?, CURRENT_TIMESTAMP)",
                               (self.current_step_id, self.state))
                conn.commit()

        def update_ui(self):
            _, total_steps, step_name, order_sequence = self._get_step_info()
            step_text = f"({order_sequence}/{total_steps}) {self.replace_placeholders(step_name, self.placeholders)}"
            ...
            self.screen.ids.steps_completed_label.text = step_text
    ...
    def next_step(self):
        ...
        if next_step_id is not None:
            self.step.current_step_id = next_step_id
            self.step.order_sequence = next_order_sequence
            self.step.change_state("Started")
        else:
            # If there are no more steps, go back to tasks screen or handle accordingly
            self.go_to_tasks()
